...
public class Trie{
	...
	public void addToMap(HashMap<Integer, ArrayList<Integer>> hmap, int matches, int rindex){

		if(!hmap.containsKey(matches)){
			ArrayList<Integer> temp = new ArrayList<Integer>();
			temp.add(rindex);
			hmap.put(matches, new ArrayList(temp));
		}
		else{
			ArrayList<Integer> temp = hmap.get(matches);
			temp.add(rindex);
	...
	public ArrayList<Integer> solve(String A, ArrayList<String> B) {
		...
		for(int i=0; i<keywords.length; i++){
			//System.out.println("Inserting " + keywords[i]);
			insertString(keywords[i], root);
		}			
		...
		for(int i=0; i<B.size(); i++){
			...
			for(int k=0; k<words.length; k++){
				if(searchString(words[k], root)){
					matches++;
					//System.out.print("Matched " + words[k] + " ");
				}
			}
		...
		for(int i=maxmatches; i>=minmatches; i--){
			if(hmap.containsKey(i)){
				ArrayList<Integer> tempi = hmap.get(i);
				res.addAll(tempi);	
			}
